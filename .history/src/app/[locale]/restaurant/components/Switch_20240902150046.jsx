...
export default function Switch({ activeTab, setactiveTab }) {
    ...
    useEffect(() => {
        ...
        setIndicatorPosition(activeTab === 'Floors' ? '0%': activeTab === "Menu" ? '20%' : activeTab === 'Promotions' ? '50%' : '75%');
        };
    ...
    return (
        <div className="relative bg-[#EFF0F1] py-2 px-4 rounded-full border border-[#E6E6E6] text-gray-80 w-fit flex items-center">
            ...
            <div
                className={`absolute z-10 bottom-0 w-[100px] h-full bg-[#346CC4] rounded-full flex items-center justify-center pointer-events-none transition-left duration-300 ease-in-out`}
                style={{
