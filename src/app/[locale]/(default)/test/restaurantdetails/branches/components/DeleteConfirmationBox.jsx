...
const DeleteConfirmationBox = ({ isOpen, onClose, onDelete }) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-[758px] h-[313px] sm:h-auto md:h-[313px] m-2 bg-white rounded-lg shadow-lg p-6 flex flex-col">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mt-auto">
          <button onClick={onClose} className="w-full md:w-full text-[#F34141] px-4 py-2 border border-[#F34141] rounded-lg">
            Cancel
          </button>
          <button onClick={onDelete} className="w-full md:w-full bg-[#F34141] text-white px-4 py-2 rounded-lg">
            Delete
          </button>
        </div>
      </div>
