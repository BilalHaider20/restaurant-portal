        ...
        <div className="relative bg-[#EFF0F1] py-2 px-4 rounded-full border border-[#E6E6E6] text-gray-80 w-fit flex items-center">
            <div className="flex justify-between gap-7 text-sm font-medium z-20  ">
                <button
                    ...
                    className={`text-${activeTab === 'Floors' ? 'white' : 'gray-80'} w-[128px] transition-all delay-100 text-xs md:text-sm lg:text-base xl:text-lg`}
                >
                    Floors
                </button>
                <button
                    onClick={() => handleClick('Menu')}
                    className={`text-${activeTab === 'Menu' ? 'white' : 'gray-80'} w-[128px] transition-all delay-100 text-xs md:text-sm lg:text-base xl:text-lg`}
                >
                    Menu
                </button>
                <button
                    onClick={() => handleClick('Promotions')}
                    className={`text-${activeTab === 'Promotions' ? 'white' : 'gray-80'} w-[128px] transition-all delay-100 text-xs md:text-sm lg:text-base xl:text-lg`}
                >
                    Promotions
                </button>
                <button
                    onClick={() => handleClick('Members')}
                    className={`text-${activeTab === 'Members' ? 'white' : 'gray-80'} w-[128px] transition-all delay-100 text-xs md:text-sm lg:text-base xl:text-lg`}
                >
                    Members
                </button>
            </div>
