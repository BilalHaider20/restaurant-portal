...
const RestaurantModal = ({ onClose }) => {
  ...
  return (
    <div className="w-full h-[1123px] max-w-3xl bg-[#F2F4F7] min-h-screen absolute right-0 mx-auto transition-all ease-linear transform">
        ...
        <div>
          ...
          <div className="border-2 border-line rounded-lg p-8 text-center">
            ...
            <p className="text-[#1F2937]">
              {t('dragFiles')} 
              <button className="text-blue-700" onClick={handleBrowseClick}>{t('browse')}</button>
              <input 
                type="file"
                ref={fileInputRef}
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white p-4 flex justify-end space-x-4">
        <button className="px-4 py-2 text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-100" onClick={onClose}>
          {t('cancel')}
        </button>
